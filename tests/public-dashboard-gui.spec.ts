...
test("task selection preserves deep links and browser history without inventing pending ranks", async ({ page }) => {
  ...
  await selectTask(page, 0); await expect(detail.getByRole("table", { name: "Returned recommendations" })).toBeVisible();
  ...
  await page.reload(); await expect(detail.getByRole("table", { name: "Returned recommendations" })).toBeVisible();
