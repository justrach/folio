...
test("actual D1 ensure races commit one request-linked provider start and preserve retries, cache and ambiguity across restart", { timeout: 90_000 }, async () => {
  ...
  try {
    ...
    assert.equal((await getAgentObservation(db, "alice", input, { ...env, OPENAI_AGENTS_MODEL: "changed-model" })).run?.model, "gpt-6-luna", "Open-web observations default to Luna independently of legacy configuration.");
