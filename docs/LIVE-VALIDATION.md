...
The initial model comparison preserved all 40 outcomes and used completed attempts for plotted medians; failures remained in cost totals. That public model comparison has since been replaced by the website comparison described below. Public rankings add 33 allowlisted observations to the 286-observation source artifact (319); separately shared D1 observations are merged by the production read API. Local checks include 23 focused unit/artifact tests, 16 public dashboard browser cases, four ranking browser cases, and two desktop/mobile landscape cases. Those browser checks use mocked network responses; the 40-run cohort above is separate actual provider work.


## 18 September 2026: website comparison across the core questions

The follow-up collection fills missing model/question pairs across the original 35 public website questions. It does not execute the expanded 3,850-question library. Existing successful answers are reused, and the frozen authorization contains 85 new attempts: Astra 3, Sol 27, Terra 27 and Luna 28. The managed open-web harness is unchanged; optional TypeSafe and DataForSEO tools are off. Failed and ambiguous creation attempts are not automatically retried.

The completed batch produced **76 validated public answers, eight failed output/evidence validations and one unresolved initial creation**. The unresolved attempt has no confirmed provider session receipt; it retains unknown outcome/cost and its accounting hold. No duplicate initial input was sent. All 85 authorized entries were attempted and the runner exited; no pending queue remains. Read-only retrieval recovered nine delayed usage estimates. The other 84 attempts now have estimates in private D1; complete billed provider charges remain distinct from estimates.

The resulting core chart has **131/140 saved model/question answers and 67 recommended websites**: Astra 35/35, Sol 34/35, Terra 32/35 and Luna 30/35. The published artifact has 395 observations across the broader index; two separately shared D1 observations are merged by the public API. These totals include earlier observations, not 395 new runs. Missing model answers remain visibly unmeasured.

The default chart now plots websites across all core questions, with category, question and model filters. Appearance rates are calculated within each model's available answers and then averaged with equal weight per measured model. Recommendation position is averaged across recommending models, conditional on appearance. Missing answers are excluded and coverage is shown; incomplete question coverage can still affect the comparison. Each website opens a model breakdown with links to actual source answers. Reviewed documentation aliases are grouped, while unrelated shared-host websites remain separate.

These are recorded research-agent recommendations within a curated question set. Some questions name websites explicitly. They are not universal website-quality scores, independent factual judgments, Google rankings or measurements of consumer AI products. Raw responses, owner/session identifiers and usage receipts remain private. Only validated, allowlisted public observations are committed.

Validation: 36 focused unit/artifact tests and two desktop/mobile interaction cases passed against the final published data. TypeScript passed. Browser checks cover category and model filters, website search/details, source-answer links, history and refresh, absence of paid POSTs, and no page-level horizontal overflow. These local checks are separate from the 85 real provider attempts above.
