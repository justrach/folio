...
## Authenticated workflow check — 17 September 2026

An existing signed-in production account completed the actual preview → publish → reopen → withdraw → reload path for the Resend email-setup observation. The reviewed projection contained question metadata, model/harness, ordered recommendations, citations and limitations. Publication persisted across browser sessions; withdrawal persisted after reload. The public GET retained 3,500 questions and 286 observations because this observation already had an independent reviewed static copy. Withdrawing the owner's new copy does not remove that preexisting operator publication. No private captures, raw answers, reference facts or provider receipts were added to Git.

An older website answer failed the publication evidence gate, as intended; no incompatible observation was published. The live account still reports two active attempts: one has unconfirmed creation without a session ID, and one has a recorded session with an unconfirmed cancellation outcome. Refreshing the originals did not establish completion or cancellation. No new model run was started, no limit was changed, and unknown cost remains unknown. The finite Luna validation remains pending those operational constraints.

The live check also exposed a misleading cancellation control for attempts with no recorded session. The report now omits that control and explains that saved-state checks cannot retrieve or cancel an unidentified remote task. Known-session cancellation remains distinct from terminal cancellation.


The same authenticated workflow saved a new three-question website suite using the editable audience/task drafts. The saved target, questions, language and locale survived reload; all three remained explicitly not started. The existing active-run safeguard kept new starts disabled. This validates drafting and persistence, not provider answers or publication of those questions.

## Website history and model coverage

The private overview now chooses an exact recorded model, labels every answer/attempt with its model, and preserves the selected website, scope and model through navigation and login. All models means latest completed answer per question across models, not a pooled vote. The history API verifies the owned website and filters its exact saved URL, search mode and optional model before returning a bounded page. Stable timestamp/ID cursors expose older attempts; model options come from the complete matching website/scope history. Other websites cannot crowd out this site's results. Legacy suite/run list callers retain their existing contract.

Partial history is labeled as partial: a question without a loaded attempt is not declared never run until matching history is fully loaded. Pagination errors keep the loaded results visible. Changing website/model clears previous selection coverage, and owner changes retain the existing remount/abort isolation. No navigation or history read starts inference.

Validation for scoped history/recovery: 267 unit tests, 26 isolated D1 tests, 20 desktop/mobile overview checks and four recovery browser checks passed. The D1 regression includes 205 synthetic attempts, equal-timestamp cursor ties, another website, another scope, older model discovery and cross-owner denial. The GUI regression loads an older answer from a second page and checks that incomplete history never says it was not run. TypeScript and the production OpenNext build passed. These fixture checks are separate from the authenticated publication/draft checks above.

Deployed scoped-history/recovery release: Worker `af232adf-2e58-4733-ab2a-32edcb3e8e35` on 17 September 2026, preserving production secrets/variables and the separate landing/index workers. The authenticated production overview loaded the existing completed answers plus all three newly saved draft questions; exact Astra selection survived reload. Anonymous scoped-history GET returned401, and public GET remained no-store with 3,500 questions/286 observations. Browser locator timeouts during the live check were followed by authoritative rendered-state inspection; they were not treated as product failure or successful action by assumption.


Model-preserving index handoffs: private comparison and published-result links now carry both the exact public question ID and recorded model. A newer observation from another model must not replace the answer that prompted the link. Four desktop/mobile regression checks passed, including following the comparison into the actual dashboard with a newer competing-model fixture, reload/back behavior, and the published-result URL. These tests make no live provider calls. TypeScript passed.

The end-to-end acceptance audit still lacks completed fresh multi-model observations in the shared open-web corpus and completed answers for the newly saved website questions. Existing evidence verifies onboarding/saved-site selection, website-scoped result history, exact-context public comparison, question drafting/persistence, explicit-start safeguards, public projection/withdrawal, and returned recommendation order. Unresolved provider attempts still prevent new starts; neither a larger question catalog nor model controls alone satisfies actual multi-model collection.

Model-handoff release deployed as Worker `b25e8b3d-0dda-4b8a-8f44-3b1ed4f29c5f`. Production OpenNext build passed. An authenticated completed Resend observation linked to the public question with `model=gpt-6-astra`; following the actual link selected Astra and displayed the matching Resend recommendation at position1. No provider starts or additional publications were made. The two unresolved original attempts were refreshed again and remained unresolved.

## Sol and Terra sandbox choices — 18 September 2026

Open-web observations now offer Sol (`gpt-5.6-sol`) and Terra (`gpt-5.6-terra`) alongside Astra and Luna. Both new choices remain experimental pending a completed managed-provider run. The selected model is frozen before creation and retained in reviewed publication projections. Astra remains the default; the shared active-run limit, hosted environment, disabled shell network, and experimental-model SEO restrictions are unchanged.

Validation: 23 focused unit tests, three isolated D1 model-selection checks, six desktop/mobile launch checks, and TypeScript passed. A recovery regression additionally checks that an idle session with empty history retains the precise unresolved-initial-submission explanation even when a cancellation was previously requested. Cancellation state and active capacity remain intact.

Read-only provider investigation found the known unresolved session idle with no turns, items, or requested actions. A complete 324-session project listing had no exact run-metadata matches for the unidentified saved attempts. Absence from that listing does not establish zero cost or a terminal outcome. No replacement creation, new inference, session deletion, limit change, or private provider evidence publication was performed. Live Sol/Terra execution remains blocked by unresolved existing attempts; local checks do not establish provider compatibility.

The final nine-test isolated D1 file and production OpenNext build passed. This release was pushed as `eec149c` and deployed as Worker `51710560-3d3e-44a5-b256-9044e1f3a1c0`; production renders both experimental model choices. Only reviewed source, tests, and this operational summary were committed. Private diagnostics and generated browser reports remain outside Git.

September 18 update: Luna is now the default. Explicit SEO-enabled website questions support bounded related-keyword research and the existing domain overview across the supported models. The earlier Astra-only restriction above is superseded; live tool connectivity still requires separate validation. See [keyword research](KEYWORD-RESEARCH.md).
