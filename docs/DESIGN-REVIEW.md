...
Every website appears in the searchable table, loaded 12 rows at a time. A point or row opens a per-model breakdown. Chart labels highlight leading sites and the selected site. Explicit catalog documentation aliases map to canonical website identities; shared-host subdomains remain distinct. Costs are not part of this public chart. Desktop/mobile tests exercise source-question selection, browser history/refresh, website drill-downs, and no paid actions from navigation.
