...
Historical design, superseded by the website comparison below.
...
## Website index correction — 18 September 2026
The public index compares websites, not model operating costs. Each chart mark is a recommended hostname. For one exact question, use the latest published answer per model; show appearance share and mean recommendation position conditional on appearance. Models filter evidence. Preserve answer denominators and exclude missing/failed answers rather than scoring them as absences. The adjacent table exposes overlapping points and serves narrow screens. Cost accounting remains separate from this public experience.

## Website averages — 18 September 2026
The default landscape includes all 35 core website questions, with audience, question and model filters stored in the URL. For each model, calculate website appearances divided by that model's saved answers in the selection. The headline appearance rate averages these model rates with equal weight. Position averages each recommending model's mean position, conditional on appearance. Missing models remain unknown. Show per-model answer coverage beside the chart and in website drill-downs; unequal coverage is explicitly partial. The overall question mix contains named websites and is not a universal website-quality ranking.

Every website appears in the searchable table, loaded 12 rows at a time. A point or row opens a per-model breakdown, with a link to an actual recommending source answer for each model. The observed date range is visible. Chart labels highlight leading sites and the selected site. Explicit catalog documentation aliases map to canonical website identities; shared-host subdomains remain distinct. Costs are not part of this public chart. Desktop/mobile tests exercise source-question selection, browser history/refresh, website drill-downs, and no paid actions from navigation.
