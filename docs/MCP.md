...
Receipt discovery runs at startup and at five-minute checkpoints after any provider cooldown. A checkpoint pauses new starts and drains current jobs under the existing exclusive runner lock before touching the saved queue. Discovery is bounded to 40 pages of 100 sessions and 30 seconds; incomplete pagination cannot establish a unique match and attaches nothing. Already known receipts are reconciled with GETs only. Actual terminal evidence may free capacity; merely recovering a session ID does not. After the checkpoint the scheduler recomputes all owner usage against the same 20-slot ceiling. Exhausted capacity triggers one final safe checkpoint before stopping if capacity remains unavailable. An upstream circuit-breaker stop is not reset by recovery.
