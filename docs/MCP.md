...
- `folio_observation`: `run`/`compatibleRun` preserve strict current-configuration reuse semantics. `latestCompletedRun` independently exposes historical evidence; `currentAttempt`, `historyState` and compatibility reasons distinguish no history, incomplete attempts, incompatible history and stale results. Paid reuse remains unchanged.
...
- `folio_visibility_compare`: non-overlapping baseline/comparison windows (exclusive ends), optional case/model/search-mode filters, exact compatible question/harness/environment/locale pairing and evidence IDs. Missing pairs are unmeasured. Each window loads at most 100 attempts with separate cursors and explicit coverage; for larger comparisons clients must collect history pages and pair across the complete dataset. Caller-supplied publication timestamps are not independently verified and pre-publication observations are flagged. No causal claim or automatic retest is made.
...
MCP target schemas express distinct website/keyword alternatives. Successful structured content uses `result`; errors use an `error` object with stable `code`, safe `message`, `retryDisposition` and `requiredScope`. Invalid input, insufficient scope and missing records are distinct. Uncertain execution requires inspecting saved state and retaining the same request key. Output schemas advertise both envelopes. Provider secrets are never included.
