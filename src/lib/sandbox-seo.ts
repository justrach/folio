...
export async function createSandboxSeoGrant(db: D1Database, ownerId: string, run: KeywordBenchmarkRun, env: SandboxSeoEnvironment) {
  ...
  const inserted = await db.prepare(`INSERT INTO sandbox_seo_grants(id,user_id,run_id,token_hash,domain,expires_at,created_at,tool_version)
    SELECT ?,?,?,?,?,?,?,2 WHERE EXISTS(SELECT 1 FROM keyword_benchmark_runs WHERE id=? AND user_id=? AND status='queued' AND create_attempt_at IS NULL) RETURNING id`)
...
export type SandboxSeoPrincipal = { grantId: string; ownerId: string; domain: string; runId: string; toolVersion: number };
export async function authenticateSandboxSeoGrant(db: D1Database, token: string): Promise<SandboxSeoPrincipal> {
  ...
  const row = await db.prepare(`SELECT g.id AS grantId,g.user_id AS ownerId,g.domain,g.run_id AS runId,g.tool_version AS toolVersion
