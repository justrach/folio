...
export function websiteLandscape(observations: PublicSearchObservation[], queryId: string, model = '') {
 ...
 for(const o of latest.values()) {
  ...
  for(const r of [...o.recommendations].sort((a,b)=>a.position-b.position)) {
   ...
   const site=sites.get(domain)??{domain,name:identity?.name??r.name,url:identity?.url??r.url,positions:[]};site.positions.push(r.position);sites.set(domain,site);
  }
 }
...
export function websiteOverview(observations: PublicSearchObservation[], queryIds: string[], models: string[]) {
 ...
 type Site={domain:string;name:string;url:string;byModel:Map<string,number[]>;queries:Set<string>;firstQuery:string};
 const sites=new Map<string,Site>();
 for(const answer of answers) {
  for(const row of websiteLandscape([answer],answer.queryId,answer.model).rows) {
   const site=sites.get(row.domain)??{domain:row.domain,name:row.name,url:row.url,byModel:new Map(),queries:new Set(),firstQuery:answer.queryId};
   site.byModel.set(answer.model,[...(site.byModel.get(answer.model)??[]),row.position]);site.queries.add(answer.queryId);sites.set(row.domain,site);
  }
 }
 const rows=[...sites.values()].map(site=>{
  const perModel=coverage.map(c=>{const positions=site.byModel.get(c.model)??[];return {model:c.model,answers:c.answers,appearances:positions.length,share:c.answers?positions.length/c.answers*100:null,position:positions.length?positions.reduce((a,b)=>a+b,0)/positions.length:null}});
 ...
 return {rows,coverage,answers:answers.length,questionCount:queryIds.length,measuredModels:measured.length,complete:coverage.every(c=>c.answers===queryIds.length)&&queryIds.length>0};
}
