...
export function keywordBenchmarkErrorResponse(error: unknown) {
  ...
  if (error instanceof KeywordBenchmarkStoreError || error instanceof ScanError) return Response.json({ error: error.message }, { status: error.status, headers: PRIVATE_BENCHMARK_HEADERS });
