import { savedRows } from "./mcp-saved-evidence";
...
export function visibilityFilters(params:URLSearchParams){
  const allowed=["websiteId","startDate","endDate","platform","sourceType","limit","model","cursor"];
  ...
  return {websiteId,startDate,endDate,sourceType,limit:Number(rawLimit),model,cursor:params.get("cursor")??undefined};
}
export async function savedVisibility(db:D1Database,ownerId:string,filters:ReturnType<typeof visibilityFilters>,products?:ProductIdentity[]){
  ...
  const page=await savedRows(db,`SELECT id,created_at FROM keyword_benchmark_runs WHERE user_id=? AND json_extract(case_json,'$.targetUrl')=? AND json_extract(case_json,'$.searchMode')='open-web' AND (? IS NULL OR created_at>=?) AND (? IS NULL OR created_at<?) AND (? IS NULL OR model=?)`,[ownerId,site.url,filters.startDate,filters.startDate?Date.parse(filters.startDate):null,filters.endDate,filters.endDate?Date.parse(filters.endDate)+86400000:null,filters.model,filters.model],{limit:100,cursor:filters.cursor});
  const runs:VisibilityRun[]=[];
  // Sequential bounded reads avoid loading provider evidence for all records at once.
  for(const row of page.items){const run=await getKeywordBenchmarkRun(db,ownerId,row.id);if(run&&keywordSearchMode(run.case.searchMode)==="open-web")runs.push({id:run.id,caseId:run.caseId,suiteId:run.suiteId,case:{query:run.case.query,targetUrl:run.case.targetUrl,language:run.case.language,locale:run.case.locale,rubricVersion:run.case.rubricVersion,searchMode:run.case.searchMode},status:run.status,model:run.model,harnessVersion:run.harnessVersion,createdAt:run.createdAt,answer:run.answer?{text:"",mentions:run.answer.mentions,citations:run.answer.citations}:null});}
  const report=visibilityReport(runs,products);
  ...
  return {website:site,platform:"openai",surface:"openai-managed-agents",searchMode:"open-web",dateRange:{startDate:filters.startDate,endDate:filters.endDate,timeZone:"UTC"},coverage:{attemptsLoaded:runs.length,limit:100,truncated:page.truncated,nextCursor:page.nextCursor,unresolvedAttempts:runs.filter(r=>r.status==="requires_action").length},methodology:"Latest completed answer per question in the selected window. Exact URL host identity; unknown target identities excluded from visibility. Share of voice counts each identified domain once per answer. Positions follow returned answer order. Daily samples may contain different questions or models and are not a controlled trend. Recommendations are evidence-based rules, not verified fixes or ranking guarantees.",...report,citations:report.citations.filter(c=>filters.sourceType==="all"||c.sourceType===filters.sourceType).slice(0,filters.limit),daily:[...daily].sort(([a],[b])=>a.localeCompare(b)).map(([date,items])=>({date,...visibilityReport(items,products).current})),platforms:[{platform:"openai",models:[...new Set(runs.map(r=>r.model))],...report.current}]};
}
