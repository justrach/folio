...
export async function POST(request: Request) {
  try {
    ...
    if ((body.model !== undefined && typeof body.model !== "string") || Object.keys(body).some(key => !["caseId", "kind", "baselineRunId", "useSeoTools", "model"].includes(key)) || (body.useSeoTools !== undefined && typeof body.useSeoTools !== "boolean") || typeof body.caseId !== "string" || !["baseline", "fresh"].includes(String(body.kind)) || (body.baselineRunId !== undefined && typeof body.baselineRunId !== "string"))
      throw new KeywordBenchmarkStoreError("Choose a saved case and baseline or fresh answer.", 400);
    const run = await startKeywordBenchmark(db, ownerId, { caseId: body.caseId, kind: body.kind as "baseline" | "fresh", baselineRunId: body.baselineRunId as string | undefined, model: body.model as string | undefined }, env, { useSeoTools: body.useSeoTools === true });
