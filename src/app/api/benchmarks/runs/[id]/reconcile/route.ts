import { reconcileKeywordBenchmark } from "@/lib/keyword-benchmark-service";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    return Response.json({ run: await reconcileKeywordBenchmark(db, ownerId, id, env) }, { headers: PRIVATE_BENCHMARK_HEADERS });
