...
function Leaderboard({
  ...
  const [metric, setMetric] = useState("Overall score");
  ...
  const filtered = brands
    .filter(
      (b) =>
        (category === "All industries" || b.category === category) &&
        `${b.name} ${b.domain}`.toLowerCase().includes(query.toLowerCase()),
    )
    .map((b) => ({
      ...b,
      value:
      ...
      {indexView === "rankings" ? <RankedSearchTable /> : indexView === "html" ? <DeveloperToolsIndex /> : indexView === "published" ? (
        ...
        <>
          <div className="leaderboard-intro">
            <span>
              <FolioMark small /> THE DISCOVERY BENCHMARK
            </span>
            <p>12 sample websites. One shared standard.</p>
            <button onClick={onMethod}>
              How we evaluate <ArrowUpRight size={14} />
            </button>
          </div>
          <section className="panel leaderboard-panel">
            <div className="leaderboard-toolbar">
              <div className="chart-tabs">
                {["Overall score", "SEO health", "Discovery"].map((m) => (
                  <button
                    key={m}
                    onClick={() => setMetric(m)}
              ...
              <div className="leaderboard-filters">
                ...
                <button
                  ...
                  onClick={() =>
                    navigator.clipboard
                      .writeText(`${location.origin}/leaderboard`)
                      .then(() => onNotify("Leaderboard link copied."))
                      .catch(() =>
                        onNotify(
                          "Copy the leaderboard URL from your address bar.",
                        ),
                      )
                  }
                >
            ...
            <div className="chart-footer">
              <SampleLabel />
              <span>Illustrative scores · methodology v0.1</span>
              <span>
                POWERED BY <b>folio.</b>
              </span>
            </div>
          </section>
          <section className="panel ranking-table-panel">
            <div className="panel-heading">
              <h2>Every website has a story.</h2>
            ...
            <div className="table-scroll">
                    ...
                    >
                      ...
                      <td>
                        <button
                          className="icon-button"
                          onClick={() => setSelected(b.name)}
                          aria-label={`View ${b.name} evaluation`}
                        >
                          <ArrowUpRight size={17} />
                        </button>
                      </td>
              ...
              {!filtered.length && (
                <p className="no-results">No websites match your search.</p>
              )}
            </div>
          </section>
        </>
      ...
      {selected && (
        ...
        >
          ...
          <div className="brand-detail">
            ...
            <div>
              <h2>{selected}</h2>
              <p>{brands.find((b) => b.name === selected)?.domain}</p>
            </div>
          </div>
          <div className="detail-stats">
            {[
              {
                label: "Overall score",
                value: brands.find((b) => b.name === selected)?.score,
              },
              {
                label: "SEO health",
                value: brands.find((b) => b.name === selected)?.seo,
              },
              {
                label: "Discovery",
                value: brands.find((b) => b.name === selected)?.discovery,
              },
            ].map((s) => (
              <div key={s.label}>
                <strong>
          ...
          <p className="evidence-note plain">
            This profile uses illustrative scores for the product preview. No
            claim is made about this company’s actual performance. Published
            evaluations will include source captures, tasks, model versions,
            timestamps, and repeated trials.
          </p>
          <Button
            kind="secondary"
            onClick={() => {
          ...
          >
            Read the evaluation methodology <ArrowRight size={14} />
          </Button>
        </Dialog>
      )}
      </section>
