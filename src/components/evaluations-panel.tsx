...
import { WebsiteCrawlReport } from "./website-crawl-report";
...
function EvaluationWorkspacePanel({ initialTargetUrl, workspace, onTargetChange }: { initialTargetUrl: string; workspace: ReturnType<typeof useEvaluationWorkspace>; onTargetChange: (url: string | undefined) => void }) {
  ...
  const [websiteCrawl,setWebsiteCrawl] = useState(false);
  useEffect(()=>setWebsiteCrawl(false),[workspace.ownerId]);
  ...
  const targetAllowed = Boolean(targetHost && (websiteCrawl ? workspace.connection?.crawlAvailable : workspace.connection?.allowedTargets?.includes(targetHost)));
  ...
  async function start(rerun?: EvaluationRun) {
    ...
    try {
      ...
      const { run } = await evaluationRequest<{ run: EvaluationRun }>("/api/evaluations", {
        ...
        body: JSON.stringify({ domain: rerun?.targetUrl ?? domain.trim(), mode: "managed", ...(rerun ? { rerunOf: rerun.id } : websiteCrawl ? {websiteCrawl:true} : { ...(expectedFacts ? { expectedFacts, confirmExpectedFacts: referenceConfirmed } : {}), ...(seoReportId ? { seoReportId } : {}) }) }),
      });
    ...
    <div className={`evaluations-panel ${current || selectedId ? "has-selection" : "is-preparing"}`}>
      ...
      <section className="panel eval-suite" aria-labelledby="eval-suite-title">
        ...
        <form id="eval-launch" className="eval-launch" onSubmit={(event) => { event.preventDefault(); void start(); }}>
          ...
          <details className="eval-reference-form" hidden={websiteCrawl}>
...
export function EvaluationReport({ run, reviewOwnerId, busy = "", onRefresh, onCancel, onExport, onRerun, onDeleted, onReturnSeo, onPrepareFresh, onCompare }: {
  run: EvaluationRun; reviewOwnerId?: string; busy?: string; onRefresh?: () => void; onCancel?: () => void; onExport?: () => void; onRerun?: () => void; onDeleted?: (id: string) => void; onReturnSeo?: () => void; onPrepareFresh?: () => void; onCompare?: () => void;
}) {
  ...
  if(run.workflow==="website-crawl-v1") return <WebsiteCrawlReport run={run} busy={busy} onRefresh={onRefresh} onCancel={onCancel} onExport={onExport} onDeleted={onDeleted}/>;
  return <section className={`panel eval-report ${result ? "is-complete" : "is-running"}`} aria-labelledby="eval-report-title">
    ...
    {reviewOwnerId && run.mode === "live" && run.status === "completed" && <SemanticReview key={`${reviewOwnerId}:${run.id}`} run={run} />}
